...
const DropDownMenu: React.FC<DropdownProps> = ({ options }) => {
  ...
  return (
    <div
      className={`relative inline-block px-6 md:px-0 ${
        isOpen ? "card-shadow" : ""
      } rounded-2xl h-auto w-full`}
      ref={dropdownRef}
    >
      <div
        className={`flex items-center justify-between md:w-[272px] px-4 py-1 rounded-2xl bg-white ${
          !isOpen && "card-shadow"
        }  cursor-pointer`}
        onClick={toggleDropdown}
      >
        <span className=" font-semibold h-10 flex-center">
          {selectedOption}
        </span>
        <ArrowIcon className={`${isOpen ? "rotate-180" : ""} ml-2`} />
      </div>
      {isOpen && (
