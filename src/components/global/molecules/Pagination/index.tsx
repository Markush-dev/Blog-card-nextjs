...
import ArrowPagination from "@/components/icons/arrow-pagination";
...
const Pagination: FC<IProps> = ({
  ...
  const {
    ...
    const shouldRenderDotsBefore =
      countPage > showSteps && scalePage / showSteps > 0;
    const shouldRenderDotsAfter =
      countPage > showSteps &&
      scalePage / showSteps < Math.ceil(countPage / showSteps) - 1;
    const shouldRenderLastPageButton = !(
      countPage > showSteps && scalePage / showSteps > 0
    );
  ...
  const arrowBtnClass = (currentPage: number, page: number) => {
    ...
    return `${baseClassName} ${
      currentPage === page ? "cursor-not-allowed" : "hover:bg-gray-50"
    }`;
  };
  ...
  return (
    <div className="bg-white flex card-shadow rounded-full items-center h-12 mt-14">
      ...
      <div className="w-12 h-12 ml-4 flex-center">
        ...
        >
          <ArrowPagination className="rotate-180" />
        </button>
      </div>
    </div>
  );
